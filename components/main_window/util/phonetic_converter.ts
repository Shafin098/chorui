// This class only should be used if you want to add bangla typing without avro phonetic keyboard
// Avro keyboard and chorui generally works together well in windows, but in linux there is some issue.
export default class PhoneticConverter {
                    ...
                    if(typeof pattern.rules !== 'undefined') {
                        for(var j = 0; j < pattern.rules.length; ++j) {
                            ...
                            for(var k=0; k < rule.matches.length; ++k) {
                                ...
                                // Handle Negative
                                if(typeof (match as any).negative === 'undefined') {
                                    (match as any).negative = false;
                                    if(match.scope.charAt(0) === '!') {
                                        (match as any).negative = true;
                                ...
                                // Exact
                                    ...
                                    if(!this.isExact(match.value, fixed, s, e, (match as any).negative)) {
                                        replace = false;
                                        break;
                                    }
                                }
